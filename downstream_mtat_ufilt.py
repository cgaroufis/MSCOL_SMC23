...
# Usage: python3 downstream_ufilt.py path_to_MTAT path_to_model [--train]
...
for kk in range(0,test_numel,73):
  specs = np.empty((29*73,98,64,1))
  ct = 0
  for n in range(kk,kk+73):
  ...
  evals = np.reshape(evals,(73,29,50))
  evals_[kk:kk+73,:] = np.sum(evals,axis=1)/29
          
print(np.sum(test_labels),np.sum(evals))
